...
const CheckoutForm = () => {
	...
	const { cartItems, cartSubtotal, cartId } = location.state || {
		cartItems: [],
		cartSubtotal: 0,
	};
	...
	useEffect(() => {
		async function fetchData() {
			try {
				...
				if (
					!userResponse.data.result.userDoorNumber ||
					!userResponse.data.result.userStreetName ||
					!userResponse.data.result.userCountry ||
					!userResponse.data.result.userCity
				) {
		...
		if (!error) {
			...
			try {
				const { data } = await axios.post(
					'https://e-commerce-grupo03.onrender.com/cart/checkout',
					{
						id,
						...
						{!isEditingAddress ? (
							<>
								<p>
									...
									{userData.userDoorNumber || 'Not provided'}
								</p>
								<p>
									<strong>Street Name:</strong>{' '}
									{userData.userStreetName || 'Not provided'}
								</p>
								<p>
									<strong>Country:</strong>{' '}
									{userData.userCountry || 'Not provided'}
								</p>
								<p>
									<strong>City:</strong> {userData.userCity || 'Not provided'}
								</p>
								<button className='btnSave' onClick={handleEditAddressClick}>
									Edit Address
								</button>
							</>
						) : (
